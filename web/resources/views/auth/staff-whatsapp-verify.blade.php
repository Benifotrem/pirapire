    ...
    <div class="flex min-h-[calc(100vh-73px)] items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 px-4 py-16">
        <div class="w-full max-w-md rounded-3xl border border-slate-700 bg-slate-900/80 p-8 text-center shadow-xl">
            ...
            <p class="mt-2 text-sm text-slate-400">
                Te lo acabamos de mandar por WhatsApp. Vence en 5 minutos.
            </p>

            <form method="POST" action="{{ route('staff-whatsapp-auth.verify') }}" class="mt-6 space-y-4">
                @csrf
                <input
                    type="text"
                    inputmode="numeric"
                    ...
                    name="code"
                    placeholder="123456"
                    class="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-center text-2xl tracking-[0.5em] text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-none"
                    required
                    autofocus
                >
            ...
            <a href="{{ route('staff-login-whatsapp') }}" class="mt-6 block text-xs text-slate-500 hover:text-slate-300">
