@extends('layouts.app', ['title' => __('site.auth.login_title')])
...
@section('content')
    <div class="flex min-h-[calc(100vh-73px)] items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 px-4 py-16">
        <div class="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl">
            ...
            <h1 class="mt-4 text-2xl font-bold text-slate-900">{{ __('site.auth.login_heading') }}</h1>
            <p class="mt-2 text-sm text-slate-500">
                {{ __('site.auth.login_subtitle') }}
            </p>
            ...
            >
                {{ __('site.auth.open_wallet') }}
            </a>

            <details class="mt-4 text-left">
                <summary class="cursor-pointer text-sm text-slate-400 hover:text-slate-600">{{ __('site.auth.copy_manually') }}</summary>
