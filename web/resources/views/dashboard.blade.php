...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-lg font-semibold text-slate-900">{{ __('site.dashboard.new_alert_title') }}</h2>
            <form method="POST" action="{{ route('alerts.store') }}" class="mt-4 grid gap-4 sm:grid-cols-2">
                @csrf
                <div>
                    <label for="currency" class="block text-xs font-medium text-slate-500">{{ __('site.dashboard.currency') }}</label>
                ...
                <div>
                    ...
                    <select name="order_type" id="order_type" class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="BUY">{{ __('site.dashboard.order_type_buy') }}</option>
                        <option value="SELL">{{ __('site.dashboard.order_type_sell') }}</option>
                    </select>
                </div>
                <div>
        ...
        {{-- Alerts list ------------------------------------------------ --}}
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 class="text-lg font-semibold text-slate-900">{{ __('site.dashboard.my_alerts_title') }}</h2>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 py-3">
                        <div>
                            ...
                            <span class="text-xs {{ $alert->is_active ? 'text-emerald-600' : 'text-slate-400' }}">
                                {{ $alert->is_active ? __('site.dashboard.alert_active') : __('site.dashboard.alert_paused') }}
                            </span>
                        </div>
                        <div class="flex gap-2">
                            <form method="POST" action="{{ route('alerts.toggle', $alert) }}">
                                ...
                                <button type="submit" class="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:bg-slate-50">
                                    {{ $alert->is_active ? __('site.dashboard.pause') : __('site.dashboard.activate') }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('alerts.destroy', $alert) }}">
                                ...
                                <button type="submit" class="rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-medium text-rose-600 transition hover:bg-rose-50">
                                    {{ __('site.dashboard.delete') }}
                                </button>
                            </form>
                        </div>
                    </div>
                @empty
                    <p class="py-3 text-sm text-slate-400">{{ __('site.dashboard.no_alerts') }}</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-4 divide-y divide-slate-100">
                ...
                @empty
                    <p class="py-3 text-sm text-slate-400">{{ __('site.dashboard.no_escrow') }}</p>
                @endforelse
            </div>
        </div>
    </div>
