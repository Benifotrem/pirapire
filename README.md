...
**`ROBOSATS_API_BASE_URL` tiene que ser el `.onion` de un coordinador real** (Temple of Sats, BitcoinVeneto, etc. — la lista vigente está en `federation.md` del repo `RoboSats/robosats`), **no** el `.onion` del "cliente federado" genérico que aparece primero en cualquier búsqueda — ese host solo sirve los archivos estáticos del frontend en React; no tiene un backend Django atrás, así que cualquier ruta de API le devuelve el `index.html` de la SPA con `200 OK` en vez de un error, lo cual es fácil de confundir con "está andando" si no se mira el `Content-Type` de la respuesta. Verificado a mano contra un coordinador real antes de documentarlo: `/api/book/?currency=<índice>&type=2` sí devuelve JSON ahí.

**El parámetro `currency` no es el código ISO 4217** (eso fue un supuesto equivocado en una versión anterior de este cliente) — es un índice a la tabla interna de monedas de RoboSats, sin relación con ningún estándar externo. Esa tabla se sirve como JSON estático en `/static/assets/currencies.json` en cualquier host de RoboSats (frontend o coordinador); ahí *35* es PYG y *1* es USD — `RoboSatsClient::CURRENCY_INDEX` ya tiene esto embebido, no hace falta configurar nada aparte de la URL base. `type=2` pide compras y ventas juntas; el filtro por tipo de orden de cada alerta lo sigue haciendo `AlertMatcher` en memoria, como con cualquier otro criterio.

`App\Services\RoboSats\RoboSatsClient::fetchBook()` ya envuelve toda la llamada HTTP (proxy incluido) en un `try/catch` que nunca deja escapar la excepción: si Tor está caído o el proxy no responde, el método loggea el error y devuelve un array vacío — `RoboSatsDriver` lo traduce a "sin ofertas de esta fuente" y el agregador sigue con Mostro con normalidad. Un coordinador sin ninguna orden abierta en esa moneda responde `404` con un cuerpo JSON válido (`{"not_found": "..."}`) — eso también se trata como libro vacío, pero sin loggear un warning, porque es un estado normal (una moneda de poco volumen en un coordinador chico), no una falla. Cobertura: `tests/Unit/RoboSatsClientTest.php`, `tests/Unit/P2P/RoboSatsDriverTest.php` y `tests/Feature/PollP2POffersTest.php`.
...
- Cuando el freelancer termina, marca el trabajo como entregado y **sube su propia factura bolt11 de cobro en ese momento** (`deliver()`) — nunca se relaya un bolt11 por un chat externo, porque las facturas Lightning expiran y no se pueden generar de antemano. Opcionalmente puede adjuntar una **prueba** (`proof_path`) — una captura de pantalla u otra evidencia — para trabajos cuyo entregable es una acción verificable (compartir una publicación, dejar una reseña) en vez de un archivo o texto. Estado: **`delivered`**.
...
**Prueba de entrega (evidencia adjunta):** para micro-trabajos tipo "compartí este tuit" o "dejá esta reseña", donde el entregable no es un archivo sino la evidencia de que la acción se hizo, `deliver()` acepta opcionalmente un `proofPath` — una imagen que el freelancer sube al entregar, disponible solo desde el dashboard web y la Mini App (no desde comandos de Telegram, que quedan de solo texto). Se guarda en un disco de Laravel dedicado, `escrow-proofs` (`web/config/filesystems.php`), deliberadamente **sin** la opción `serve` que sí tiene el disco `local` — esa opción registra una ruta pública sin autenticar (`GET /storage/{path}`, ver `php artisan route:list`), que es aceptable para archivos genéricos pero no para una captura que puede contener cualquier cosa que haya salido en pantalla. En cambio, la imagen solo es accesible vía `EscrowDashboardController::proof()` / `MiniApp\CustomerController::escrowJobProof()`, que chequean que quien pide el archivo sea el creador o el freelancer asignado a ese trabajo específico antes de devolverlo (`App\Services\Escrow\EscrowProofStorage`). En la Mini App, como su autenticación es por header (`X-Telegram-Init-Data`, no por cookie de sesión), un `<img src="...">` directo no serviría — la imagen se pide por `fetch()` con ese header y se muestra vía un `blob:` URL generado en el cliente.
...
**Vidriera pública en el cartel LED (sin necesidad de cuenta):** `App\View\Composers\LedDisplayComposer::openJobAds()` mezcla los trabajos `open` en la misma rotación del cartel LED del header (`resources/views/components/led-display.blade.php`), junto con los anuncios de comercios pagados por `LedAd` — cualquier visitante, logueado o no, ve "💼 &lt;descripción&gt; — ganás N sats si lo aceptás" pasar por el cartel. Cada ítem del cartel guarda su propio destino y forma de abrirlo: un anuncio de comercio abre su URL externa en una pestaña nueva; un trabajo apunta siempre a `/dashboard/escrow` en la misma pestaña — lo que pase al clickear depende de qué ítem esté mostrándose en ese momento, no de a dónde apuntaba el anterior. Como esa ruta vive detrás de `auth:customer`, un visitante sin sesión cae directo en la pantalla de login (la invitación a loguearse que pedía el flujo) y uno ya logueado entra derecho al tablón — sin necesidad de ninguna lógica de "¿está logueado?" en el cartel mismo. Lo que hace posible esto último es que `App\Http\Controllers\Auth\LnurlAuthController::complete()` ahora respeta la URL "intended" que Laravel guarda automáticamente cuando el middleware de auth redirige a un guest — antes siempre mandaba a `/dashboard` a secas, perdiendo a dónde iba el visitante. Tests: `tests/Feature/LedDisplayTest.php` (los trabajos aparecen/desaparecen del cartel según su estado y el on/off general) y `tests/Feature/LnurlAuthTest.php` (el rebote de login respeta la URL de origen).
